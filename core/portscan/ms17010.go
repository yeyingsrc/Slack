...
func MS17010Scan(ctx context.Context, taskId, host string) error {
	...
	defer conn.Close()
	err = conn.SetDeadline(time.Now().Add(time.Duration(8) * time.Second))
	if err != nil {
		//fmt.Printf("failed to connect to %s\n", ip)
		return err
	}
	...
	if wordCount := sessionSetupResponse[0]; wordCount != 0 {
		...
		if n != int(byteCount)+45 {
			fmt.Println("[-]", host, "ms17010 invalid session setup AndX response")
		} else {
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		if reply[34] == 0x51 {
			result := fmt.Sprintf("[+] MS17-010 %s has DOUBLEPULSAR SMB IMPLANT", host)
			runtime.EventsEmit(ctx, "nucleiResult", structs.VulnerabilityInfo{
				ID:          "DOUBLEPULSAR SMB IMPLANT",
				Name:        "DOUBLEPULSAR SMB IMPLANT",
				URL:         host,
				Type:        "SMB",
				Severity:    "CRITICAL",
				Description: result,
		...
		result := fmt.Sprintf("[*] OsInfo %s\t(%s)", host, os)
		gologger.Info(ctx, result)
	}
	return err
